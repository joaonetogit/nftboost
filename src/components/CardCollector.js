...
export default function CardCollector({ index, thumbnail, name, valueBtc, percent }) {
  return (
        ...
        <div className='flex items-center gap-4 @desktop:items-center'>
          <div className='relative w-10 h-10'>
            <div className='w-full h-full rounded-full overflow-hidden flex items-center justify-center border-2 border-white border-opacity-5'>
              <Image src={thumbnail} alt='Image NFT' className='w-full h-full object-cover' />
            </div>
            <Image src={IconVerify} alt='Icon verify' className='absolute -bottom-1 -right-1' />
          </div>
          <div className='space-y-1'>
            <h4 className='font-semibold'>{name}</h4>
            <p className='flex flex-col items-start @desktop:items-center gap-1 text-sm text-white text-opacity-70 @desktop:flex-row'>Preço mínimo
